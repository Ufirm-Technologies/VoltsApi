...
using vaulterpAPI.Models.Blog;

namespace vaulterpAPI.Controllers.Blog
{
    [ApiController]
    [Route("api/blogs")]
    ...
    string.IsNullOrEmpty(model.Content) ? "{}" : model.Content
        ...
        // -----------------------------
        [HttpPut("{id}")]
        public IActionResult UpdateBlog(Guid id, [FromBody] BlogDto dto)
        {
            try
            {
                using var conn = new NpgsqlConnection(GetConnectionString());
